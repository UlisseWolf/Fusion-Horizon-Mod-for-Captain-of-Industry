...
// Minimal, dependency-free JSON translation loader. Captain of Industry mods aren't
// required to use any particular localization system, so this is a small, self-contained
// one: a flat key/value dictionary loaded from Translations/<lang>.json, with a
// language-fallback chain. Keys use a stable "<category>.<protoId>.<field>" convention
// (e.g. "recipe.Recipe_Candu_Rod.description") rather than the English text itself, so
// editing the English copy later never breaks a lookup.
	...
	public static void Initialize(string modRootPath) {
		...
		try {
			...
			if (File.Exists(candidate)) {
				...
				Log.Info($"[Fusion Horizon] Loaded translations from '{candidate}' ({s_translations.Count} keys).");
			} else {
				Log.Info("[Fusion Horizon] No translation file found, using built-in English fallback strings.");
			}
		} catch (Exception ex) {
			Log.Error($"[Fusion Horizon] Failed to load translations: {ex}");
		}
	}

	/// <summary>
	/// Looks up a structured translation key (e.g. "recipe.Recipe_Candu_Rod.name").
